...
namespace Octopus.Server.Extensibility.Authentication.DirectoryServices.IntegratedAuthentication
{
    ...
    class IntegratedAuthenticationHandler : IIntegratedAuthenticationHandler
    {
        ...
        public Task HandleRequest(HttpContext context)
        {
            ...
            var principalSessionsToken = result.Value?.SessionsToken;
            if (principalSessionsToken == null)
                throw new InvalidOperationException("The user principal's sessions token was ");

            // Build the auth cookies to send back with the response
            var authCookies = tokenIssuer.CreateAuthCookies(principalIdentificationToken.Value, principalSessionsToken.Value, TimeSpan.FromDays(20), context.Request.IsHttps, state?.UsingSecureConnection);
